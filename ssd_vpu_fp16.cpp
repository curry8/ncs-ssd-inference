...
void overlay_on_image( const Mat& image, const vector<float>& object_info) 
{
    ...
    if (percentage > min_score_percent) // ignore boxes less than the minimum score
    {
        stringstream ss;
        ss << LABELS[class_id];
        ss << percentage;
...
void initSsdNetwork() {
    ...
    if (enableSSDNetwork) {
        ...
        // allocate the graph data type fp16
        retCode = ncGraphAllocateWithFifosEx(dev_handle[0], ssd_graph_handle, gender_graph_buf, graph_len,
                                           &ssd_fifo_in, NC_FIFO_HOST_WO, 2, NC_FIFO_FP16,
                                           &ssd_fifo_out, NC_FIFO_HOST_RO, 2, NC_FIFO_FP16);
        if (retCode != NC_OK) {
...
bool getSSDResults(const Mat& inputMat, struct ncGraphHandle_t* graphHandle, struct ncFifoHandle_t* fifoIn,
    ...
    int num_valid_boxes = result_fp32[0];

    for(int i=0;i<num_valid_boxes;i++)
    {
        int base_index = 7 + i*7;
        object_info[0] = result_fp32[base_index+0];
        object_info[1] = result_fp32[base_index+1];
        object_info[2] = result_fp32[base_index+2];
        object_info[3] = result_fp32[base_index+3];
        object_info[4] = result_fp32[base_index+4];
        object_info[5] = result_fp32[base_index+5];
        object_info[6] = result_fp32[base_index+6];
